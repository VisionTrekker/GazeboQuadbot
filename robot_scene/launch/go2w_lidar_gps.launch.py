...
def generate_launch_description():
    ...
    default_model_path = os.path.join(resource_pkg_share, "xacro/go2w_description.urdf.xacro")
    default_world_path = os.path.join(resource_pkg_share, "worlds/inspection.world")
    ...
    declare_robot_name = DeclareLaunchArgument(
        "robot_name", default_value="go2w", description="Robot name"
    )
    ...
    gazebo_ld = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                get_package_share_directory("robot_scene"),
                "launch",
                "go2w_base.launch.py",
            )
        ),
    ...
    # 混合运动控制器节点
    hybrid_motion_controller_node = Node(
